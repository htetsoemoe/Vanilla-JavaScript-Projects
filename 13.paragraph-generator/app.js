...
form.addEventListener('submit', e => {
  // submint button's default event is initaites its submission to the server 
  // but we don't need that event here.
  e.preventDefault();

  const value = parseInt(amount.value);  
  // generate random number for random paragraph
  const random = Math.floor(Math.random() * text.length);

  if (isNaN(value) || value <= 0 || value > 9) {
    result.innerHTML = `<p class='result'>${text[random]}</p>`
  } else {
    // slice text array with user input value
